...
When we run a statistical test, we want to know if an observed difference---like how much more participants are willing to pay for the **special boulder** in one condition versus another --- is just **random chance** or reflects a **real effect**.
...
We need **many shuffled datasets** to accurately assess how unusual our observed difference is. Therefore, in a permutation test, we don’t rely on a single shuffle. Instead, we **repeatedly shuffle the group labels** --- thousands of times. For each shuffle, we:
...
The graph below recreates the earlier plot using 1,000 shuffled datasets. It displays the observed group means along with their variability under repeated random shuffles. While the error bars in the previous graph summarize the uncertainty **around our observed** group means (i.e. the familiar 95% confidence intervals), this plot shows what those group means would look like **if there really were no effect**. Here, each point is the average willingness‐to‐pay from one of 1,000 random reassignments of labels, and the shaded bands mark the **central 95% region** of that *null* distribution. In other words, these “permutation intervals” visualize the range of mean differences we’d expect **purely by chance** under the null hypothesis.
...
Next, **choose two conditions** to compare. The code below will assign one condition to be called Group A, and the other condition to be called Group B. By default HOPS and PI are compared, but you can change these to compare other groups.
...
observed_diff_for_graph <- diff(df_summary$mean_wtp)
...
df_summary |> 
  ...
  geom_label(
    x = 1.5, y = midpoint_y,
    label = sprintf("Difference: %.2f", observed_diff_for_graph),
...
Using permutations, we can approximate the sampling distribution of the difference in willingness to pay between the two groups under the null hypothesis. Press **Run Code** to perform 1,000 permutations, calculating the mean willingness to pay for each shuffled group in every iteration. This will take a few minutes.
...
### Take a peek at the raw data and first three shuffled datasets
...
In a permutation test, we can estimate this by calculating the **standard deviation of the mean differences** across all 1,000 shuffled datasets. This gives us the **spread of differences we’d expect under the null hypothesis**, showing the typical variation if there were **no real effect**.
...
To judge how unusual our observed difference is under the null hypothesis, we calculate a **standardized test statistic**:

1.  Take the **observed difference** in willingness to pay: $\bar{x}_B - \bar{x}_A$.
...
ggplot(std_mean_diff_permutations, aes(x = std_null_diff)) +
  ...
  xlim(min(density_df$x) - 1.25, max(density_df$x) + 1.25) +
  labs(
...
-   If the green line is **outside** the darker grey boundaries, we **can reject the null hypothesis** (indicating a significant difference between groups). In this example, such a result would suggest that the data visualization shown to participants affects their willingness to pay.
...
- $\bar{x}_A$ and $\bar{x}_B$ are the sample means for Groups A and B, respectively. The 0 in the numerator represents the null hypothesis value (i.e., no group difference).
- $SE$ is the **pooled standard error** of the difference, calculated from the sample standard deviations and sample sizes of the groups:

$$
SE = \sqrt{s_p^2 \left( \frac{1}{n_A} + \frac{1}{n_B} \right)}
$$

- $s_p^2$ is the **pooled variance**, computed as:

$$
s_p^2 = \frac{(n_A - 1)s_A^2 + (n_B - 1)s_B^2}{n_A + n_B - 2}
$$

Here, $s_A$ and $s_B$ are the sample standard deviations for Groups A and B, and $n_A$ and $n_B$ are the sample sizes for each group.
...
2.  **Built a null distribution** by shuffling labels 1,000 times, creating “permutation intervals” that show what mean differences look like purely by chance under the null hypothesis.
...
5.  **Verified your findings** with a parametric independent samples *t*-test and an equivalent linear regression, confirming that all approaches yield the same conclusion when assumptions hold.
...
-   **Vary the number of permutations.** See how the stability of your *p*-value and null‐distribution estimates changes with fewer (e.g., 500) or more (e.g., 2,000) replicates.

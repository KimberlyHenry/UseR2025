...
::: notes
For each shuffle, we calculate the difference in means. Overlaying these 5000 differences gives an empirical null distribution --- that is, our hands‑on sampling distribution under the null hypothesis.
:::
...
::: notes
We divide the observed difference by that SE to get a standardized test statistic. On the null density, we shade the central 95% and mark where our result falls --- so students see how ‘far out’ it really is.
:::
...
::: notes
webR runs fully in the browser --- no installs are needed. All code lives on GitHub, so you can drop this into any course and adapt it to new datasets in minutes.
:::
